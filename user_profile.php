    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["photo"])) {
        ...
        if (in_array($imageFileType, $allowedExtensions)) {
            if (move_uploaded_file($_FILES["photo"]["tmp_name"], $targetFile)) {
                ...
                $updateSql = "UPDATE users SET photo_path = '$targetFile' WHERE email = '$email'";
    ...
    <h1>User Profile</h1>
        <?php
            if (isset($row)) {
                ...
                echo '<p><strong>Address & Contact : </strong>' . $row['address_contact'] . '</p>';
                echo '<p><strong>Gender : </strong>' . $row['gender'] . '</p>';
                echo '<p><strong>Medical Condition : </strong>' . $row['medical_condition'] . '</p>';
                ...
                // Buttons
                echo '<a href="edit_user_profile.php"><button style="margin-top: 10px;">Edit Profile</button></a>';
                echo '<a href="user_interface01.html"><button>HOME</button></a>';
